      ...
      {rows.map((node) => {
        ...
        return (
          <div
            ...
            className="w-full"
            draggable
            // Rows nest in the DOM, so every handler stops propagation —
            // otherwise the ancestor's handler runs last and always wins.
            onDragStart={(e) => {
              e.stopPropagation();
              e.dataTransfer.setData('text/plain', node.id); // Firefox requires this
              e.dataTransfer.effectAllowed = 'move';
              drag.onDragStart(node.id);
            }}
            onDragOver={(e) => {
              if (blocked) return; // no preventDefault → browser shows "no drop"
              e.preventDefault();
              e.stopPropagation();
            ...
            <div className={`relative rounded ${isDragging ? 'opacity-40' : ''} ${marker === 'inside' ? 'bg-blue-500/10 ring-1 ring-inset ring-blue-500' : ''}`}>
              {/* Reorder gets a line; reparent gets a filled row. Different
                  affordances, or nobody can predict what a drop will do. */}
