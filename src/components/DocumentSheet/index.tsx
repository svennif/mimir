...
export function DocumentSheet({
  children, isFavorite,
}: { children: React.ReactNode; isFavorite: boolean }) {
  return (
    <div className="relative flex min-h-0 w-full flex-1 flex-col items-start overflow-clip rounded-lg border border-line bg-sheet shadow-sheet">
      ...
      <div className="flex min-h-0 w-full flex-1 items-start justify-center overflow-y-auto">
        <div className="flex w-8/12 shrink-0 flex-col items-start gap-1.5 pt-16 pb-20">
          {children}
        </div>
      </div>
    </div>
